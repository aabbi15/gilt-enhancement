...
class GiltViewProvider {
    ...
    getPrompts(selectedText, queryType, libName, nlPrompt, queryWithCode = false) {
        const fewShotExamples = {
            askGptOverview: [
                ...
                },
                {
                    ...
                    output: `This function prints a personalized greeting.`
                }
            ],
            askGptQuery: [
                {
                ...
                },
                {
                    ...
                    output: `It checks if the input is divisible by 2 to determine if it's even.`
                }
            ]
        };
        ...
        switch (queryType) {
            case "askGptOverview":
                prompt += `You are a Python code summarizer. Analyze and explain what the function below does in one clear sentence.\n\n`;
            ...
            case "askGptQuery":
                prompt += `You are a programming tutor. Answer the developer’s question based on the code below. Break down the reasoning step-by-step (chain of thought).\n\n`;
            ...
            case "askGptApi":
                prompt += `You are a Python API documentation expert. For each API used in the following code, provide a clear explanation of what it does and how it contributes to the program logic.\n\n`;
                prompt += `Code:\n${selectedText}\nExplanation:`;
                break;
    
            case "askGptConcept":
                prompt += `You are a domain expert guiding a beginner developer. Identify and explain domain-specific concepts (not APIs) that are crucial to understanding the following code.\n\n`;
                prompt += `Code:\n${selectedText}\nConcepts:`;
                break;
    
            case "askGptUsage":
                prompt += `You are an AI assistant helping a developer understand how to use specific APIs. Based on the code snippet, generate a complete example that shows the correct usage of the APIs used.\n\n`;
                prompt += `Code:\n${selectedText}\nExample:`;
                break;
